-- Ejecutar conectado a sga_facultad_ciencias_educacion. Crea el nodo físico Facultad de Ciencias y Educación.
CREATE EXTENSION IF NOT EXISTS pgcrypto;

CREATE SCHEMA facultad_ciencias_educacion;

CREATE TABLE facultad_ciencias_educacion.estudiante (
    id_estudiante uuid PRIMARY KEY,
    id_usuario uuid NOT NULL UNIQUE,
    id_facultad varchar(10) NOT NULL DEFAULT 'FCE',
    id_programa_academico varchar(20) NOT NULL,
    limite_matriculas int NOT NULL DEFAULT 15,
    estado_estudiante varchar(20) NOT NULL,
    CHECK(id_facultad='FCE'),
    CHECK(limite_matriculas>0),
    CHECK(estado_estudiante IN ('Activo','Inactivo','Suspendido','Retirado','Egresado'))
);

CREATE TABLE facultad_ciencias_educacion.profesor (
...
CREATE TABLE facultad_ciencias_educacion.administrativo (
...
CREATE TABLE facultad_ciencias_educacion.programa_asignatura (
...
CREATE TABLE facultad_ciencias_educacion.pre_requisito (
    ...
    FOREIGN KEY(id_programa_asignatura,id_facultad) REFERENCES facultad_ciencias_educacion.programa_asignatura(id_programa_asignatura,id_facultad),
    FOREIGN KEY(id_programa_asignatura_requisito,id_facultad) REFERENCES facultad_ciencias_educacion.programa_asignatura(id_programa_asignatura,id_facultad)
);

CREATE TABLE facultad_ciencias_educacion.grupo (
    ...
    FOREIGN KEY(id_programa_asignatura,id_facultad) REFERENCES facultad_ciencias_educacion.programa_asignatura(id_programa_asignatura,id_facultad),
    FOREIGN KEY(id_profesor) REFERENCES facultad_ciencias_educacion.profesor(id_profesor)
);

CREATE TABLE facultad_ciencias_educacion.matricula (
    ...
    FOREIGN KEY(id_estudiante) REFERENCES facultad_ciencias_educacion.estudiante(id_estudiante)
);

CREATE TABLE facultad_ciencias_educacion.inscripcion (
    ...
    FOREIGN KEY(id_grupo,id_facultad) REFERENCES facultad_ciencias_educacion.grupo(id_grupo,id_facultad),
    FOREIGN KEY(id_estudiante) REFERENCES facultad_ciencias_educacion.estudiante(id_estudiante)
);

CREATE TABLE facultad_ciencias_educacion.homologacion (
    ...
    FOREIGN KEY(id_profesor_evaluador) REFERENCES facultad_ciencias_educacion.profesor(id_profesor),
    FOREIGN KEY(id_estudiante) REFERENCES facultad_ciencias_educacion.estudiante(id_estudiante),
    FOREIGN KEY(id_programa_asignatura,id_facultad) REFERENCES facultad_ciencias_educacion.programa_asignatura(id_programa_asignatura,id_facultad)
);

CREATE TABLE facultad_ciencias_educacion.decano (
...
CREATE TABLE facultad_ciencias_educacion.coordinador (
