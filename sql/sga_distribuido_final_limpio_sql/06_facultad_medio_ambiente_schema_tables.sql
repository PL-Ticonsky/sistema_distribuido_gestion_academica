-- Ejecutar conectado a sga_facultad_medio_ambiente. Crea el nodo físico Facultad de Medio Ambiente y Recursos Naturales.
CREATE EXTENSION IF NOT EXISTS pgcrypto;

CREATE SCHEMA facultad_medio_ambiente;

CREATE TABLE facultad_medio_ambiente.estudiante (
    id_estudiante uuid PRIMARY KEY,
    id_usuario uuid NOT NULL UNIQUE,
    id_facultad varchar(10) NOT NULL DEFAULT 'FMA',
    id_programa_academico varchar(20) NOT NULL,
    limite_matriculas int NOT NULL DEFAULT 15,
    estado_estudiante varchar(20) NOT NULL,
    CHECK(id_facultad='FMA'),
    CHECK(limite_matriculas>0),
    CHECK(estado_estudiante IN ('Activo','Inactivo','Suspendido','Retirado','Egresado'))
);

CREATE TABLE facultad_medio_ambiente.profesor (
...
CREATE TABLE facultad_medio_ambiente.administrativo (
...
CREATE TABLE facultad_medio_ambiente.programa_asignatura (
...
CREATE TABLE facultad_medio_ambiente.pre_requisito (
    ...
    FOREIGN KEY(id_programa_asignatura,id_facultad) REFERENCES facultad_medio_ambiente.programa_asignatura(id_programa_asignatura,id_facultad),
    FOREIGN KEY(id_programa_asignatura_requisito,id_facultad) REFERENCES facultad_medio_ambiente.programa_asignatura(id_programa_asignatura,id_facultad)
);

CREATE TABLE facultad_medio_ambiente.grupo (
    ...
    FOREIGN KEY(id_programa_asignatura,id_facultad) REFERENCES facultad_medio_ambiente.programa_asignatura(id_programa_asignatura,id_facultad),
    FOREIGN KEY(id_profesor) REFERENCES facultad_medio_ambiente.profesor(id_profesor)
);

CREATE TABLE facultad_medio_ambiente.matricula (
    ...
    FOREIGN KEY(id_estudiante) REFERENCES facultad_medio_ambiente.estudiante(id_estudiante)
);

CREATE TABLE facultad_medio_ambiente.inscripcion (
    ...
    FOREIGN KEY(id_grupo,id_facultad) REFERENCES facultad_medio_ambiente.grupo(id_grupo,id_facultad),
    FOREIGN KEY(id_estudiante) REFERENCES facultad_medio_ambiente.estudiante(id_estudiante)
);

CREATE TABLE facultad_medio_ambiente.homologacion (
    ...
    FOREIGN KEY(id_profesor_evaluador) REFERENCES facultad_medio_ambiente.profesor(id_profesor),
    FOREIGN KEY(id_estudiante) REFERENCES facultad_medio_ambiente.estudiante(id_estudiante),
    FOREIGN KEY(id_programa_asignatura,id_facultad) REFERENCES facultad_medio_ambiente.programa_asignatura(id_programa_asignatura,id_facultad)
);

CREATE TABLE facultad_medio_ambiente.decano (
...
CREATE TABLE facultad_medio_ambiente.coordinador (
